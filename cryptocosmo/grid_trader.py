...
class GridTrader:
    ...
    def sync(
        ...
        if allow_new_buys:
            self._place_grid_buys(connector, price, balances, order_size_multiplier)
    ...
    def _place_grid_buys(
        ...
        # Apply RSI-derived multiplier (clamped so we never place zero-size orders)
        effective_order_size = max(
            self.cfg.grid.order_size * order_size_multiplier,
            self.cfg.grid.order_size * 0.1,  # never below 10% of base size
        )
        ...
        for lvl in self.buy_levels:
            ...
            if already_open:
                continue
            if exposure + lvl * effective_order_size > max_exposure:
                continue
            try:
                order = connector.place_limit_buy(self.symbol, lvl, effective_order_size)
                gid = str(order.get("id"))
                self.active_orders[gid] = GridOrder(level_price=lvl, side="buy", amount=effective_order_size, order_id=gid)
                exposure += lvl * effective_order_size
                logging.info(
                    "Placed buy @ %.2f size=%.6f mult=%.2f (exposure now %.2f)",
                    lvl, effective_order_size, order_size_multiplier, exposure,
                    extra={"action": "place_buy"},
                )
